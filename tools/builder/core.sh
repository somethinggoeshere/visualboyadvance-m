...
DISTS=$DISTS'
    ...
    meson           https://github.com/mesonbuild/meson/releases/download/0.43.0/meson-0.44.0.tar.gz            bin/meson
...
unpack_needed_dists() {
    ...
    for current_dist in $DIST_NAMES; do
        if ! path_exists "$(dist_dir "$current_dist")"; then
            (
                start_job
                write_job_info dist_name $current_dist
                {
...
build_dist() {
    ...
    if [ -n "$build_override" ]; then
        ...
        if [ "$config_type" = meson ] || [ -f meson.build ]; then
            mkdir -p build
            cd build
            ...
            path_exists "$(install_artifact $current_dist)"
        elif [ "$config_type" = autoconf -o "$config_type" = autoreconf ] || [ -f configure -o -f Configure -o -f configure.ac -o -f configure.in -o -f Makefile.am ]; then
            ...
            path_exists "$(install_artifact $current_dist)"
        elif [ "$config_type" = cmake ] || [ -f CMakeLists.txt ]; then
            mkdir -p build
            cd build
            ...
            path_exists "$(install_artifact $current_dist)"
        elif [ "$config_type" = python ] || [ -f setup.py ]; then
            if [ -z "$install_override" ]; then
                pip=
            ...
            path_exists "$(install_artifact $current_dist)"
        elif [ "$config_type" = perl ] || [ -f Makefile.PL ]; then
            ...
            path_exists "$(install_artifact $current_dist)"
        elif [ "$config_type" = make ] || [ -f Makefile -o -f makefile ]; then
            makefile=makefile
            if [ -f Makefile ]; then
                makefile=Makefile
            fi
